...
const dispatch = useDispatch();
    function onRegister(event) {
        ...
        const email = JSON.stringify(event.target[0].value);
        const password = JSON.stringify(event.target[2].value);
       dispatch (authAction(email, password, date, dateForRegisterUser))
    }
    ...
    return (
        ...
        ?  navigate('/contacts')
        : <div className={'login'}>
                <Box onSubmit={onRegister} className={'form-control'} component={'form'} validate>
